...
public class StreamsExample {
    ...
    public static void main(final String[] args) {
        ...
        Consumer<Author> activeAuthorConsumer = new Consumer<Author>() {
            @Override
            public void accept(Author author) {
                System.out.println(author);
            }
        };
        Predicate<Author> activeAuthorFilter = new Predicate<Author>() {
            @Override
        ...
        // TODO With functional interfaces declared
        Function<Author, Stream<Book>> getAuthorBooksStream = new Function<Author, Stream<Book>>() {
            @Override
            public Stream apply(Author author) {
                return author.books.stream();
            }
        };
        Predicate<Book> publishedBooksFilter = new Predicate<Book>() {
            @Override
            public boolean test(Book book) {
                return book.published;
            }
        };
        Consumer<Book> printBookDetails = new Consumer<Book>() {
            @Override
        ...
        authors.stream()
                .flatMap(getAuthorBooksStream)
                .filter(publishedBooksFilter)
                .forEach(printBookDetails);
        ...
        OptionalDouble averageBookPrice = authors.stream()
                .flatMap(getAuthorBooksStream)
                .mapToInt(getBookPrice)
                .average();
        averageBookPrice.ifPresent(System.out::println);

        banner("Average price for all books in the library - lambda");
        // TODO With functional interfaces used directly
        averageBookPrice = authors.stream()
                        .flatMap(author -> author.books.stream())
        ...
        Predicate<Author> hasAtLeastOnePublishedBook = new Predicate<Author>() {
            @Override
            public boolean test(Author author) {
                return author.books.stream().anyMatch(publishedBooksFilter);
            }
        };
        authors.stream()

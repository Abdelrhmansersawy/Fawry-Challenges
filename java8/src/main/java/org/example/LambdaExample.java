...
public class LambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        Predicate<User> olderThanFiveFilter = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age > 5;
            }
        ...
        repository.select(olderThanFiveFilter , sortByNameComparator);
        ...
        // TODO With functional interfaces declared
        Predicate<User> youngerThanTenFilter = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age < 10;
            }
        ...
        repository.select(youngerThanTenFilter , sortByAgeComparator);
        ...
        // TODO With functional interfaces declared
        repository.select(null , sortByNameComparator);

        banner("Listing active users sorted by name - lambda");
        // TODO With functional interfaces used directly
        repository.select(null , Comparator.comparing(u -> u.name));

        banner("Listing active users with age > 8 sorted by name");
        // TODO With functional interfaces declared
        Predicate<User> olderThanEightFilter = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age > 8;
            }
        };
        repository.select(olderThanEightFilter , sortByNameComparator);
